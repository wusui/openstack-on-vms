...
sudo -t ssh ${VMADDR} cat /root/.ssh/id_rsa.pub >> /tmp/everybody
sudo -t ssh ${VMADDR} cat ${HOME}/.ssh/id_rsa.pub >> /tmp/everybody
...
sudo -t ssh ${VMADDR} cp /tmp/everybody /root/.ssh/authorized_keys
sudo -t ssh ${VMADDR} cp /tmp/everybody ${HOME}/.ssh/authorized_keys
