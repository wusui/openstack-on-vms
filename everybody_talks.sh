...
sudo ssh -t ${VMADDR} cat /root/.ssh/id_rsa.pub >> /tmp/everybody
sudo ssh -t ${VMADDR} cat ${HOME}/.ssh/id_rsa.pub >> /tmp/everybody
...
sudo ssh -t ${VMADDR} cp /tmp/everybody /root/.ssh/authorized_keys
sudo ssh -t ${VMADDR} cp /tmp/everybody ${HOME}/.ssh/authorized_keys
