...
sudo subscription-manager repos --enable=rhel-7-server-rpms --enable=rhel-7-server-extras-rpms --enable=rhel-7-server-openstack-${osversion}-rpms --enable=rhel-7-server-openstack-${osversion}-director-rpms --enable=rhel-7-server-rh-common-rpms
sudo yum update -y
